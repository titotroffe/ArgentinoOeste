...
// Run with: npx sanity exec src/sanity/migrate-initial-data.ts --with-user-token
    ...
    cuerpo: [
        ...
        },
        {
            ...
            children: [
                {
                    _type: 'span',
                    text: 'Corría el minuto 88 y el empate 0-0 parecía sentenciado. Fue entonces cuando apareció la figura de Juan "La Fiera" Rodríguez. Recibió un pase filtrado, eludió al arquero y con el arco libre desató la locura. El grito de gol se escuchó hasta las vías del tren.',
                },
            ],
        },
    ],
...
async function runMigration() {
    ...
    try {
        const res = await client.createOrReplace(gloriaEternaNote)
        console.log('Nota creada con éxito:', res._id)
    } catch (err) {
